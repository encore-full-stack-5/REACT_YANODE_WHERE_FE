...
export default function menu() {
  ...
  const getdata = async () => {
    try {
      const response = await axios.get(
        "http://192.168.80.39:3001/users/stores/all/10"
      );
  ...
  const isOpen = (e) => {
    ...
    console.log(nowSplit);
    console.log(openSplit);
    console.log(closeSplit);

    nowSplit[0] = String(
      Number(nowSplit[0] == 12 ? 0 : nowSplit[0]) +
        (nowRaw.split(" ")[0] == "오후" ? 12 : 0)
    );
    console.log(nowSplit[0]);
          ...
          <ul className={common.srchList}>
            <li className={[common.srch, common.full].join(" ")}>
              <div className={common.inptWrap}>
                <label>매장 이름</label>
                <input type={"type"} className={common.inpt} />
              </div>
